...
class CITESeqData(UnimodalData):
    _matrix_keywords = [
        ...
        "raw.count",
        "arcsinh.jitter",
    ]  # 'arcsinh.jitter' is in dense format, np.ndarray
    _uns_keywords = [
        "_other_names",
        "_other_counts",
    ]  # '_other' are antibodies that set aside
    ...
    def __getitem__(self, index: INDEX) -> UnimodalDataView:
        ...
        return UnimodalDataView(
            self, barcode_index, feature_index, self._cur_matrix, obj_name="CITESeqData"
        )
    ...
    def set_aside(self, params: List[str]) -> None:
        ...
        self.metadata["_other_names"] = self.feature_metadata.index.values[
            locs
        ]  # with loc: List[int], this should be a copy not a reference
    ...
    def arcsinh_transform(
        ...
        if "raw.count" not in self.matrices:
            raise ValueError(
                "raw.count matrix must exist in order to calculate the arcsinh transformed matrix!"
            )
        ...
        if jitter:
            ...
            self.matrices["arcsinh.jitter"] = np.arcsinh(
                signal / cofactor, dtype=np.float32
            )
        else:
