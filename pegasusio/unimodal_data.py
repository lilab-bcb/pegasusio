...
class UnimodalData:
    def __init__(
        ...
        if len(self.barcode_metadata) > 0:
            if isinstance(self.barcode_metadata, MutableMapping):
                ...
                self.barcode_metadata = pd.DataFrame(
                    data=self.barcode_metadata,
                    index=pd.Index(barcodekey, name="barcodekey"),
                )
            else:
                if not isinstance(self.barcode_metadata, pd.DataFrame):
                    raise ValueError(
                        f"Unknown barcode_metadata type: {type(self.barcode_metadata)}!"
                    )
        ...
        if len(self.feature_metadata) > 0:
            ...
            if not isinstance(self.feature_metadata, pd.DataFrame):
                raise ValueError(
                    f"Unknown feature_metadata type: {type(self.feature_metadata)}!"
                )
        ...
        if cur_matrix not in matrices.keys():
            raise ValueError(
                "Cannot find the default count matrix to bind to. Please set 'cur_matrix' argument in UnimodalData constructor!"
            )
        ...
        for key in list(metadata.keys()):
            ...
            if isinstance(value, np.ndarray) and value.ndim == 2:
                ...
                isinstance(value, csr_matrix) and value.shape[0] == value.shape[1]
            ):  # is a graph
    ...
    def _gen_repr_str_for_attrs(self, key: str) -> str:
        if key == "obs" or key == "obsm":
            ...
            for attr in getattr(self, key):
                ...
                attr_str += (
                    f"'{attr}', " if attr_type is None else f"'{attr}'({attr_type}), "
                )
            return attr_str[:-2]
        else:
    ...
    def _update_shape(self) -> None:
        self._shape = (
            self.barcode_metadata.shape[0],
            self.feature_metadata.shape[0],
        )  # shape

    def _is_dirty(self) -> bool:
        """Check if any field is modified."""
        return (
            self.matrices.is_dirty()
            or self.metadata.is_dirty()
            or self.barcode_multiarrays.is_dirty()
            or self.feature_multiarrays.is_dirty()
            or self.barcode_multigraphs.is_dirty()
            or self.feature_multigraphs.is_dirty()
        )
    ...
    def get_attr_type(self, attr: str) -> str:
        ...
        if ("_attr2type" not in self.metadata) or (
            attr not in self.metadata["_attr2type"]
        ):
            return None
    ...
    def _polish_featurekey(self, feature_metadata: pd.DataFrame, genome: str) -> None:
        ...
        if genome != "unknown":
            ...
            if prefix.match(feature_metadata["featurekey"][0]):
                feature_metadata["featurekey"] = np.array(
                    [prefix.sub("", x) for x in feature_metadata["featurekey"].values],
                    dtype=object,
                )
                if "featureid" in feature_metadata:
                    feature_metadata["featureid"] = np.array(
                        [
                            prefix.sub("", x)
                            for x in feature_metadata["featureid"].values
                        ],
                        dtype=object,
                    )
        ...
        if feature_metadata["featurekey"].duplicated().sum() > 0:
            ...
            ids = (
                feature_metadata["featureid"].values
                if "featureid" in feature_metadata
                else None
            )
            ...
            for i in range(keys.size):
                ...
                if idn > 1:
                    keys[i] = (
                        keys[i] + f".#~{idn}"
                    )  # duplicate ID starts from 2. .#~ makes it unique.
    ...
    def add_matrix(self, key: str, mat: csr_matrix) -> None:
        ...
        if mat.shape[0] != self._shape[0]:
            raise ValueError(
                f"Wrong number of barcodes: matrix '{key}' has {mat.shape[0]} barcodes, which does not match with the barcode file ({self._shape[0]})!"
            )
        if mat.shape[1] != self._shape[1]:
            raise ValueError(
                f"Wrong number of features: matrix '{key}' has {mat.shape[1]} features, which does not match with the feature file ({self._shape[1]})!"
            )
    ...
    def _inplace_subset_obs(self, index: List[bool]) -> None:
        ...
        for key in list(self.barcode_multigraphs):
            self.barcode_multigraphs[key] = self.barcode_multigraphs[key][index][
                :, index
            ]
    ...
    def _inplace_subset_var(self, index: List[bool]) -> None:
        ...
        for key in list(self.feature_multigraphs):
            self.feature_multigraphs[key] = self.feature_multigraphs[key][index][
                :, index
            ]
    ...
    def separate_channels(self) -> None:
        ...
        if (
            self.barcode_metadata.shape[0] == 0
            or self.barcode_metadata.index[0].find("-") < 0
        ):
        ...
        if np.unique(channels).size > 1:
            ...
            barcodes = np.array(
                [x + "-" + y for x, y in zip(channels, barcodes)], dtype=object
            )
    ...
    def from_anndata(
        self, data: anndata.AnnData, genome: str = None, modality: str = None
    ) -> None:
        ...
        if "gene_ids" in self.feature_metadata:
            self.feature_metadata.rename(
                columns={"gene_ids": "featureid"}, inplace=True
            )

        self.matrices = DataDict(
            {"X": csr_matrix(data.X)}
        )  # csr_matrix will not copy X.data
            ...
            for key, value in old_dict.items():
                if str(type(value)).find("anndata") >= 0:
                    ...
                    if is_dict_like(value):
                        ...
                        logger.warning(
                            f"{key} is in anndata-defined data type {type(value)} and thus skipped!"
                        )
                else:
                    new_dict[key] = value
        ...
        self.barcode_multigraphs = _create_data_dict(
            data.obsp
        )  # Note that we do not check if data.obsp contains sparse matrix
        self.feature_multigraphs = _create_data_dict(
            data.varp
        )  # Note that we do not check if data.obsp contains sparse matrix
    ...
    def to_anndata(self) -> anndata.AnnData:
        ...
        if "raw.X" in self.matrices:
            ...
            raw = anndata.AnnData(
                X=self.matrices["raw.X"], var=self.feature_metadata[var_cols]
            )
    ...
    def _convert_attributes_to_categorical(self, attributes: Set[str]) -> None:
        for attr in attributes:
            ...
            self.barcode_metadata[attr] = pd.Categorical(
                values, categories=natsorted(np.unique(values))
            )
