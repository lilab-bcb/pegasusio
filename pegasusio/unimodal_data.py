...
from pegasusio import fast_funcs
...
class UnimodalData:
    ...
    def separate_channels(self) -> None:
        ...
        barcodes, channels = fast_funcs.split_barcode_channel(self.barcode_metadata.index.values)
    ...
    def from_anndata(self, data: anndata.AnnData) -> None:
        ...
        self.matrices = {"X": data.X}
        if data.raw is not None:
            self.matrices["raw.X"] = data.raw.X
        if len(data.layers) > 0:
            self.matrices.update(dict(data.layers))
