...
class VDJDataView(UnimodalDataView):
    def __init__(
        ...
        super().__init__(
            vdjdata, barcode_index, feature_index, cur_matrix, obj_name="VDJData"
        )
    ...
    def get_chain(self, chain: str) -> pd.DataFrame:
        ...
        df = pd.DataFrame(
            data=data, index=self.obs_names, columns=self.parent._matrix_keywords
        )
...
class VDJData(UnimodalData):
    ...
    _features = {
        "tcr": ["TRA", "TRB", "TRD", "TRG", "Multi"],
        "bcr": ["IGK", "IGL", "IGH", "Multi"],
    }
    _n_contigs = 10
    ...
    def __init__(
        ...
        for chain in VDJData._features[self.uns["modality"]]:
            ...
            self.obs["n" + chain] = self.X[:, pos : pos + VDJData._n_contigs].getnnz(
                axis=1
            )
    ...
    def get_chain(self, chain: str) -> pd.DataFrame:
        ...
        df = pd.DataFrame(
            data=data, index=self.obs_names, columns=VDJData._matrix_keywords
        )
    ...
    def construct_clonotype(self, min_umis: int = 2) -> None:
        if self.get_modality() == "tcr":
            ...
            self.obs["clonotype"] = ""
            self.obs.loc[idx, "clonotype"] = (
                df1.loc[idx, "cdr3"] + "|" + df2.loc[idx, "cdr3"]
            )
            ...
            if idx2.sum() > 0:
                ...
                self.obs.loc[idx2, "clonotype"] = (
                    df3.loc[idx2, "cdr3"] + "|" + df4.loc[idx2, "cdr3"]
                )
        else:
            ...
            self.obs["clonotype"] = ""
            self.obs.loc[idxkh, "clonotype"] = (
                df2.loc[idxkh, "cdr3"] + "|" + df1.loc[idxkh, "cdr3"]
            )
            self.obs.loc[idxlh, "clonotype"] = (
                df3.loc[idxlh, "cdr3"] + "|" + df1.loc[idxlh, "cdr3"]
            )
