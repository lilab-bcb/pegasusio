...
def load_10x_vdj_file(
    input_csv: str, genome: str = None, modality: str = None
) -> MultimodalData:
    ...
    df = pd.read_csv(
        input_csv, na_filter=False
    )  # Otherwise, '' will be converted to NaN
    idx = df["productive"] == (True if df["productive"].dtype.kind == "b" else "True")
    df = df[idx]
    df.sort_values(
        by=["barcode", "umis"], ascending=[True, False], inplace=True, kind="mergesort"
    )  # sort barcode and make sure it is stable
    ...
    if (modality is not None) and (modality != modal):
        raise ValueError(
            f"Detected modality '{modal}' does not match user-provided modality '{modality}'!"
        )
    modality = modal
    ...
    feature_metadata = {
        "featurekey": [
            x + (str(y + 1) if y > 0 else "")
            for x, y in itertools.product(
                VDJData._features[modality], range(VDJData._n_contigs)
            )
        ]
    }
    fid2pos = {}
    ...
    barcodes, is_cell, mats, strarrs = convert_10x_vdj_to_vdjdata(
        df["barcode"].values,
