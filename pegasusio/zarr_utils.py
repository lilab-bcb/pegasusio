...
class ZarrFile:
    def __init__(self, path: str, mode: str = "r", storage_type: str = None) -> None:
        ...
        if mode == "w":
            ...
            self.store = (
                zarr.ZipStore(path, mode="w")
                if storage_type == "ZipStore"
                else zarr.NestedDirectoryStore(path)
            )
            ...
            self.store = (
                ...
                if os.path.isdir(path)
                else zarr.ZipStore(path, mode="r")
            )
    ...
    def read_series(
        self, group: zarr.Group, name: str
    ) -> Union[pd.Categorical, np.ndarray]:
        if "ordered" in group[name].attrs:
            # categorical column
            return pd.Categorical.from_codes(
                group[name][...],
                categories=group[f"_categories/{name}"][...],
                ordered=group[name].attrs["ordered"],
            )
        else:
    ...
    def read_dataframe(self, group: zarr.Group) -> pd.DataFrame:
        ...
        )  # if add columns = columns, the generation will be slow
        return df
    ...
    def read_unimodal_data(self, group: zarr.Group) -> UnimodalData:
        ...
        if DataClass == SpatialData:
            unidata = DataClass(
                ...
                img=self.read_dataframe(group["img"])
                if "img" in group
                else dict(),  # for backward-compatibility
            )
        else:
    ...
    def write_series(self, group: zarr.Group, name: str, array: np.ndarray) -> None:
        if (
            not is_categorical_dtype(array)
            and name != "_index"
            and is_string_dtype(array)
        ):
            keywords = set(array)
        ...
        if is_categorical_dtype(array):
            ...
            # write codes
            codes_arr = group.create_dataset(
                name,
                data=array.codes,
                shape=array.codes.shape,
                chunks=calc_chunk(array.codes.shape),
    ...
    def write_record_array(
        self, parent: zarr.Group, name: str, array: np.recarray
    ) -> None:
        group = parent.create_group(name, overwrite=True)
    ...
    def write_composite_list(
        self, parent: zarr.Group, name: str, values: Union[list, tuple]
    ) -> None:
        group = parent.create_group(name, overwrite=True)
        scalar_dict = {}
    ...
    def write_mapping(
        self, parent: zarr.Group, name: str, mapping: dict, overwrite=True
    ) -> None:
        group = None
        if overwrite:
            group = parent.create_group(name, overwrite=True)
        else:
            group = parent.require_group(
                name, overwrite=False
            )  # throw an error if name in array_keys()
    ...
    def write_unimodal_data(
        self, parent: zarr.Group, name: str, data: UnimodalData, overwrite: bool = True
    ) -> None:
        """Write UnimodalData
        ...
        if overwrite or data.feature_multigraphs.is_dirty():
            self.write_mapping(
                group,
                "feature_multigraphs",
                data.feature_multigraphs,
                overwrite=overwrite,
            )

    def write_multimodal_data(
        self, data: MultimodalData, overwrite: bool = True
    ) -> None:
        """Write MultimodalData"""
        if overwrite:
            for key in data.list_data():
                self.write_unimodal_data(
                    self.root, key, data.get_data(key), overwrite=True
                )
        else:
            ...
            for key in data.data.accessed:
                self.write_unimodal_data(
                    ...
                    key,
                    data.get_data(key),
                    overwrite=key in data.data.modified,
                )
