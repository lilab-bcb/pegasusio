...
def _enumerate_files(
    path: str, parts: List[str], repl_list1: List[str], repl_list2: List[str] = None
) -> str:
...
def _locate_barcode_and_feature_files(path: str, fname: str) -> Tuple[str, str]:
    ...
    if fname == "matrix":
        barcode_file = _enumerate_files(
            path, [""], ["cells.tsv.gz", "cells.tsv", "barcodes.tsv.gz", "barcodes.tsv"]
        )
        feature_file = _enumerate_files(
            path, [""], ["genes.tsv.gz", "genes.tsv", "features.tsv.gz", "features.tsv"]
        )
    else:
        ...
        if p2 == "" and p3 == "":
            ...
            barcode_file = _enumerate_files(
                path, [p1, "", p3, ""], ["barcodes", "cells"], [".tsv.gz", ".tsv"]
            )
            feature_file = _enumerate_files(
                path, [p1, "", p3, ""], ["genes", "features"], [".tsv.gz", ".tsv"]
            )
...
def _load_barcode_metadata(
    barcode_file: str, sep: str = "\t"
    ...
    if "cellkey" in barcode_metadata.iloc[0].values:
        ...
        barcode_metadata = pd.DataFrame(
            data=barcode_metadata.iloc[1:].values,
            columns=barcode_metadata.iloc[0].values,
        )
...
def _load_feature_metadata(
    feature_file: str, format_type: str, sep: str = "\t"
) -> Tuple[pd.DataFrame, str]:
    ...
    if format_type == "HCA DCP":
        ...
        feature_metadata = pd.DataFrame(
            data=feature_metadata.iloc[1:].values, columns=series.values
        )
        ...
        if feature_metadata.shape[1] > 1:
            ...
            if feature_metadata.iloc[0, 0].find("_") >= 0:
                ...
                feature_metadata = pd.DataFrame(
                    data={"featureid": arr[:, 0], "featurekey": arr[:, 1]}
                )
            else:
...
def load_one_mtx_file(
    path: str, file_name: str, genome: str, modality: str
) -> UnimodalData:
    ...
    if file_name.endswith(".gz"):
        ...
        os.mkfifo(mtx_fifo)
        subprocess.Popen(
            f"gunzip -c {shlex.quote(mtx_file)} > {shlex.quote(mtx_fifo)}", shell=True
        )
...
def load_mtx_file(
    path: str, genome: str = None, modality: str = None
) -> MultimodalData:
    ...
    if os.path.isdir(orig_file):
        ...
        if (not orig_file.endswith(".mtx")) and (not orig_file.endswith(".mtx.gz")):
            raise ValueError(
                f"File {orig_file} does not end with suffix .mtx or .mtx.gz!"
            )
    ...
    if file_name is not None:
        if genome is None:
            genome = "unknown"
        data.add_data(
            load_one_mtx_file(path, file_name, genome, modality),
        )
    else:
        for dir_entry in os.scandir(path):
            if dir_entry.is_dir():
                ...
                if file_name is None:
                    raise ValueError(
                        f"Folder {dir_entry.path} does not contain a mtx file!"
                    )
                dgenome, dmodality = _parse_dir_name(dir_entry.name, modality)
                data.add_data(
                    load_one_mtx_file(dir_entry.path, file_name, dgenome, dmodality)
                )
...
def _write_mtx(unidata: UnimodalData, output_dir: str, precision: int):
    ...
    for key in unidata.list_keys():
        ...
        mtx_file = os.path.join(
            output_dir, ("matrix" if key == "X" else key) + ".mtx.gz"
        )
        ...
        pobj = subprocess.Popen(
            f"gzip < {shlex.quote(fifo_file)} > {shlex.quote(mtx_file)}", shell=True
        )
        write_mtx(
            fifo_file,
            matrix.data,
            ...
            matrix.shape[1],
            precision=precision,
        )  # matrix is cell x gene csr_matrix, will write as gene x cell
        assert pobj.wait() == 0
        os.unlink(fifo_file)
        logger.info(f"{mtx_file} is written.")

    unidata.barcode_metadata.to_csv(
        os.path.join(output_dir, "barcodes.tsv.gz"), sep="\t"
    )
    logger.info("barcodes.tsv.gz is written.")

    unidata.feature_metadata.to_csv(
        os.path.join(output_dir, "features.tsv.gz"), sep="\t"
    )
...
def load_csv_file(
    ...
    if os.path.isfile(barcode_file) and os.path.isfile(feature_file):
        ...
        feature_metadata, format_type = _load_feature_metadata(
            feature_file, format_type, sep=sep
        )
    ...
    if input_csv.endswith(".gz"):
        ...
        os.mkfifo(csv_fifo)
        subprocess.Popen(
            f"gunzip -c {shlex.quote(input_csv)} > {shlex.quote(csv_fifo)}", shell=True
        )
        row_ind, col_ind, data, shape, rowkey, rownames, colnames = read_csv(
            csv_fifo, sep
        )
        os.unlink(csv_fifo)
    else:
    ...
    if rowkey == "cellkey":
        # HCA format
        assert (
            (barcode_metadata is not None)
            and (feature_metadata is not None)
            and (barcode_metadata.shape[0] == shape[0])
            and (feature_metadata.shape[0] == shape[1])
            and (
                (barcode_metadata["barcodekey"].values != np.array(rownames)).sum() == 0
            )
            and (
                (feature_metadata["featureid"].values != np.array(colnames)).sum() == 0
            )
        )
        mat = csr_matrix((data, (row_ind, col_ind)), shape=shape)
    else:
        ...
        if barcode_metadata is None:
            ...
            assert (barcode_metadata.shape[0] == shape[1]) and (
                (barcode_metadata["barcodekey"].values != np.array(colnames)).sum() == 0
            )
        if feature_metadata is None:
            feature_metadata = {"featurekey": rownames}
        else:
            assert (feature_metadata.shape[0] == shape[0]) and (
                (feature_metadata["featurekey"].values != np.array(rownames)).sum() == 0
            )
...
def _write_scp_metadata(
    unidata: UnimodalData, output_name: str, precision: int = 2
) -> None:
...
def _write_scp_expression(
    unidata: UnimodalData, output_name: str, is_sparse: bool, precision: int = 2
) -> None:
    ...
    if is_sparse:
        ...
        gene_ids = (
            unidata.var["featureid"].values
            if "featureid" in unidata.var
            else (unidata.var["gene_ids"] if "gene_ids" in unidata.var else gene_names)
        )

        df = pd.DataFrame({"gene_names": gene_names, "gene_ids": gene_ids})[
            ["gene_ids", "gene_names"]
        ]
        ...
        write_dense(
            expr_file,
            unidata.obs_names.values,
            unidata.var_names.values,
            matrix.data,
            matrix.indices,
            matrix.indptr,
            matrix.shape[0],
...
def write_scp_file(
    data: MultimodalData, output_name: str, is_sparse: bool = True, precision: int = 2
) -> None:
