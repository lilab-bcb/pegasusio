...
class CytoData(UnimodalData):
    _matrix_keywords = [
        ...
        "raw.data",
        "arcsinh.jitter",
    ]  # all in dense format, np.ndarray
    _obsm_keywords = [
        "_parameters"
    ]  # _parameters store parameters that are not used, e.g. Time
    ...
    def __getitem__(self, index: INDEX) -> UnimodalDataView:
        ...
        return UnimodalDataView(
            self, barcode_index, feature_index, self._cur_matrix, obj_name="CytoData"
        )
    ...
    def set_aside(self, params: List[str] = ["Time"]) -> None:
        ...
        self.metadata["_parameter_names"] = self.feature_metadata.index.values[
            locs
    ...
    def arcsinh_transform(
        ...
        if "raw.data" not in self.matrices:
            raise ValueError(
                "raw.data matrix must exist in order to calculate the arcsinh transformed matrix!"
            )
