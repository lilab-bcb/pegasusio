...
class AggrData:
    ...
    @run_gc
    def _merge_matrices(
        self, feature_metadata: pd.DataFrame, unilist: List[UnimodalData], modality: str
    ) -> Dict[str, csr_matrix]:
        ...
        for unidata in unilist:
            ...
            if no_reorg and (
                feature_metadata.shape[0] > unidata.feature_metadata.shape[0]
                or (feature_metadata.index != unidata.feature_metadata.index).sum() > 0
            ):
                no_reorg = False
            ...
            for mat_key in mat_keys:
                ...
                for unidata in unilist:
                    ...
                    if mat is not None:
                        mat_list.append(mat)
                matrices[mat_key] = (
                    vstack(mat_list) if modality != "cyto" else np.vstack(mat_list)
                )
        else:
            colmap = []
            for unidata in unilist:
            ...
            if modality == "cyto":
                ...
                for mat_key in mat_keys:
                    ...
                    for i, unidata in enumerate(unilist):
                        ...
                        if mat is not None:
                            newmat = np.zeros(
                                (mat.shape[0], feature_metadata.shape[0]),
                                dtype=mat.dtype,
                            )
                            newmat[:, colmap[i]] = mat
                ...
                for mat_key in mat_keys:
                    ...
                    matrices[mat_key] = coo_matrix(
                        (data, (row, col)), shape=(row_base, feature_metadata.shape[0])
                    ).tocsr(copy=False)

        return matrices

    @run_gc
    def _vdj_update_metadata_matrices(
        self, metadata: dict, matrices: dict, unilist: List[UnimodalData]
    ) -> None:
        ...
        for key in VDJData._uns_keywords:
            ...
            for unidata in unilist:
                ...
                old2new = np.array(
                    [val2num[val] for val in unidata.metadata[key]], dtype=np.int32
                )
    ...
    def _aggregate_unidata(self, unilist: List[UnimodalData]) -> UnimodalData:
        ...
        barcode_metadata = pd.concat(
            barcode_metadata_dfs, axis=0, sort=False, copy=False
        )
        ...
        for other in unilist[1:]:
            ...
            feature_metadata = feature_metadata.merge(
                other.feature_metadata, on=keys, how="outer", sort=False, copy=False
        ...
        if isinstance(unilist[0], CITESeqData):
            unidata = CITESeqData(
                barcode_metadata, feature_metadata, matrices, metadata
            )
